...
static std::vector<listElement> generateExportableList(const clientRequest& client_req, std::vector<listElement>& full_list, const std::vector<std::string>& hash) {
	std::vector<listElement> listToReturn;
	listToReturn.reserve(15000);
	std::vector<listElement> listToShuffle = full_list;
	auto rng = std::default_random_engine{};
	

	std::shuffle(std::begin(listToShuffle), std::end(listToShuffle), rng);
